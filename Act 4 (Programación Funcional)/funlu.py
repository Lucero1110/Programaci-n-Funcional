...
def sustitución_int(data):
    import pandas as pd
    #Separo columnas cuantitativas del dataframe
    cuantitativas_con_nulos = data.select_dtypes(include='float64')
    #Separo columnas cualitativas del dataframe
...
#9 Identificar valorres nulos por una columna en específico y por tipo de dato en específico 

def cuenta_nulos(data,col):
    Data_Type = data[col].dtype
    if (Data_Type == "int64") | (Data_Type == "floar64"):
    #Primer método de sustitución de valores nulos
    #Sustituir valores nulos con promedio o media
        data[col] = data[col].fillna(round(data[col].mean(), 1))
        return data
    else: 
        raise ValueError(f"la variable no es númerica, es de tipo: {Data_Type}")
